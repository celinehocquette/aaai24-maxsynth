...
import pickle
import itertools
from . util import format_rule, prog_size, format_prog, flatten, reduce_prog, prog_is_recursive, prog_has_invention, \
...
class Combiner:
    ...
    def update_prog_index(self, prog, pos_covered, neg_covered):
        ...
        self.saved_progs[len(neg_covered)+prog_size(prog)].add(tuple(ids))
    ...
    def update_best_prog(self, new_progs, timeout=None):
        ...
        pos_covered, neg_covered = self.tester.test_prog(new_solution)
        tp = len(pos_covered)
        fp = len(neg_covered)
