...
class Explainer:
    ...
    def explain_totally_incomplete(self, prog, directions, noise=False):
        return self.explain_totally_incomplete_aux(prog, directions, 0, set(), set(), noise=noise)

    def explain_totally_incomplete_aux(self, prog, directions, depth, sat=set(), unsat=set(), noise=False):
        ...
        for subprog in find_subprogs(prog, has_recursion):
            ...
            if headless:
                ...
                if self.tester.is_sat(test_prog, noise=noise):
                    if not noise:
                        sat.add(raw_prog)
                    continue
            ...
            if not noise:
                unsat.add(raw_prog)
            xs = list(self.explain_totally_incomplete_aux(subprog, directions, depth+1, sat, unsat, noise=noise))
            if len(xs):
                yield from xs
            else:
