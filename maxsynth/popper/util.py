...
def parse_args():
    ...
    parser.add_argument('--nonoise', default=False, action='store_true', help='tell Popper that there is no noise')
    ...
    parser.add_argument("--specialisation-prune", default=True, action='store_false',
                        help='EXPERIMENTAL FEATURE: prune specialisation of programs which have worse score than one of their generalisation')
    parser.add_argument("--batch-combine", default=True, action='store_true', help='EXPERIMENTAL FEATURE: call the combine stage every change of size')
    parser.add_argument("--delete-combine", default=False, action='store_true', help='EXPERIMENTAL FEATURE: delete some rules from the combine stage')
    parser.add_argument("--constraints", default=True, action='store_false',help='EXPERIMENTAL FEATURE: ignore examples in the combine stage which are not covered by any program')
...
class Stats:
    ...
    def show(self):
        ...
        message += f'Pruned programs: {self.pruned_count}\n'
        message += f'Deleted programs: {self.deleted_count}\n'
...
def print_prog_score(prog, score):
    ...
    print(f'Precision:{precision} Recall:{recall} TP:{tp} FN:{fn} TN:{tn} FP:{fp} Size:{size} Score:{size+fn+fp}')
...
def order_rule_datalog(rule, settings):
    ...
    with settings.stats.duration("order"):
        head, body = rule
        ordered_body = []
        seen_vars = set()
...
class Settings:
    def __init__(self, cmd_line=False, info=True, debug=False, show_stats=True, bkcons=False, max_literals=MAX_LITERALS,
        ...
        if cmd_line:
            ...
            batch_combine = args.batch_combine
            delete_combine = args.delete_combine
        ...
        self.delete_combine = delete_combine
    ...
    def print_incomplete_solution(self, prog, tp, fn, tn, fp, size):
        self.logger.info('*'*20)
